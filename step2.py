"""
Step 2: 2D multi-UAV environment with obstacles and collision detection.

Completed so far:
- Step 1: reset() / step() multi-UAV kinematics.
- Step 2: add circular obstacles, workspace boundary check, obstacle collision check,
          inter-UAV collision check, and simple rendering utilities.

Still NOT included in this step:
- Targets and arrival judgment.
- Dynamic target assignment.
- LiDAR observation.
- DA-MAPPO reward function.

This file can be saved as: envs/uav_env.py
"""
...
class MultiUAV2DEnv:
    """
    Multi-UAV 2D environment with circular obstacles and collision detection.
    ...
    Observation per UAV in Step 2:
        [x_norm, y_norm, cos(heading), sin(heading), v_norm, omega_norm]

    Later steps will extend the observation to match the paper:
        o_i = [LiDAR z_i, ego-motion u_i, assigned-target feature g_i, swarm topology q_i]
    """

    def __init__(self, config: Optional[UAVEnvConfig] = None):
        ...
        # Obstacles are represented by centers [x, y] and radii.
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
        ...
        Args:
            actions: np.ndarray with shape [num_agents, 2]
                     actions[:, 0] = linear velocity commands
                     actions[:, 1] = angular velocity commands

        Returns:
            next_obs: np.ndarray, shape [num_agents, obs_dim]
            rewards:  np.ndarray, shape [num_agents]
                      Step 2 uses a simple placeholder reward:
                      -1 per step, -100 if terminated by collision or boundary.
                      Full DA-MAPPO rewards will be added later.
            dones:    np.ndarray, shape [num_agents]
                      All agents share episode termination.
            info:     dict with diagnostics
        """
        if self.done:
            # Avoid silently stepping a finished episode.
            return self._get_obs(), np.zeros((self.num_agents,), dtype=np.float32), np.ones((self.num_agents,), dtype=bool), self._get_info()
        ...
        # Track traveled distance for later evaluation.
        ...
        self.done = bool(boundary_violation or obstacle_collision or inter_agent_collision or timeout)
        ...
        rewards = np.full((self.num_agents,), -1.0, dtype=np.float32)
        if boundary_violation or obstacle_collision or inter_agent_collision:
            rewards[:] = -100.0
    ...
    def _reset_uavs(self) -> None:
        ...
        for i in range(self.num_agents):
            ...
            self.headings[i] = 0.0  # Facing positive x direction
    ...
    def _generate_obstacles(self) -> None:
        ...
        The method is simple rejection sampling. It is enough for the early reproduction
        stage and can be replaced by fixed benchmark maps later.
        """
        centers = []
        radii = []
        max_attempts = 10_000
    ...
    def _check_obstacle_collision(self) -> bool:
        ...
        # distances shape: [num_agents, num_obstacles]
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        obs = np.stack(
            [x_norm, y_norm, cos_h, sin_h, v_norm, omega_norm],
            axis=1,
        ).astype(np.float32)

        return obs

    def _get_info(self, **extra_flags: bool) -> Dict:
    ...
    def render_state(self) -> None:
        ...
        for i in range(self.num_agents):
            ...
            print(
                ...
                f"v={self.linear_velocities[i]:.2f}, "
                f"omega={self.angular_velocities[i]:.2f}"
            )
    ...
    def render_matplotlib(self, ax=None):
        ...
        # Draw UAVs and headings.
        ax.scatter(self.positions[:, 0], self.positions[:, 1], marker="o")
...
if __name__ == "__main__":
    cfg = UAVEnvConfig(num_agents=3, num_obstacles=10, seed=42)
    env = MultiUAV2DEnv(cfg)

    obs = env.reset()
    print("Initial obs shape:", obs.shape)
    env.render_state()
    print("Obstacle centers shape:", env.obstacle_centers.shape)

    for _ in range(400):
        # Example: all UAVs fly forward with a small turn.
        actions = np.array(
            [
                [1.0, 0.8],
                [1.0, 0.0],
