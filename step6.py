"""
Step 6: Trainable simple reward for DA-MAPPO reproduction.
...
Step 6 adds a cleaner simple reward that is suitable for early training:
    1. Individual progress reward
    2. Team distance reward
    3. Arrival reward
    4. Obstacle proximity penalty from LiDAR
    5. Inter-UAV proximity penalty
    6. Acceleration smoothness penalty
    7. Step penalty
    8. Failure penalty

This is not the full paper reward yet. It is a stable intermediate reward for
getting small scenes to train before implementing the complete hierarchical reward.
"""
...
class MultiUAV2DEnv:
    ...
    def _compute_simple_trainable_rewards(
        ...
        This reward is intentionally simpler than the full paper reward, but it
        preserves the main learning signals:
    ...
    def _compute_lidar_proximity_penalty(self) -> np.ndarray:
        ...
        Uses the minimum LiDAR distance per UAV:
        - below danger distance: fixed strong penalty;
        - between danger and warning distance: smooth penalty;
        - beyond warning distance: no penalty.
        """
