...
class MultiUAV2DEnv:
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
        ...
        # Keep it fixed for reward / arrival computation to avoid assignment-induced
        # distance-change contamination.
        assignments_for_reward = self.assignments.copy()
        previous_distances = self._compute_distances_for_assignments(assignments_for_reward)

        prev_positions = self.positions.copy()
        target_positions_before_dynamic_update = self.target_positions.copy()
        ...
        # Dynamic targets are part of the environment transition from s_t to s_{t+1}.
        self._maybe_update_dynamic_targets()

        # Reward and success are computed using the same assignment that generated the action.
        current_distances = self._compute_distances_for_assignments(assignments_for_reward)
        ...
        info = self._get_info(
            ...
            target_positions_before_dynamic_update=target_positions_before_dynamic_update.copy(),
        )
    ...
    def get_global_state(self) -> np.ndarray:
        ...
        state_parts = [
            ...
            self.target_velocities.reshape(-1) / max(self.cfg.target_speed, 1e-6),
