"""
Step 5: Paper-style observation for DA-MAPPO reproduction.
...
This version keeps the pluggable online target assignment module from Step 4,
and replaces the temporary observation with the paper-style observation:

    o_i = [z_i, u_i, g_i, q_i]

where:
    z_i: 2D LiDAR distances, shape [lidar_num_rays]
    u_i: ego-motion state [v, omega, a_v, a_omega]
    g_i: assigned-target feature [distance, relative_bearing]
    q_i: teammate relative positions, shape [2 * (num_agents - 1)]

Current observation dimension:
    obs_dim = lidar_num_rays + 4 + 2 + 2 * (num_agents - 1)

Still NOT included in this step:
- Dynamic target movement / swapping.
- Full DA-MAPPO hierarchical reward.
- MAPPO neural network and training loop.
"""
...
class UAVEnvConfig:
    ...
    # LiDAR settings, matching the paper's D=35 setting by default.
...
class MultiUAV2DEnv:
    """
    Multi-UAV 2D environment with paper-style observations and pluggable assignment.
    ...
    def step(self, actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
        ...
        rewards = self._compute_placeholder_rewards(
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        lidar_obs = self._compute_lidar_observations()       # z_i
        ego_obs = self._compute_ego_motion_observations()    # u_i
        target_obs = self._compute_target_observations()     # g_i
        topology_obs = self._compute_topology_observations() # q_i

        obs = np.concatenate([lidar_obs, ego_obs, target_obs, topology_obs], axis=1)
        if obs.shape != (self.num_agents, self.obs_dim):
            raise RuntimeError(f"Observation shape mismatch: got {obs.shape}, expected {(self.num_agents, self.obs_dim)}.")
    ...
    def _compute_lidar_observations(self) -> np.ndarray:
        ...
        This LiDAR only scans static circular obstacles. Inter-UAV collision is handled
        separately by the environment and not included in z_i at this step.
        """
            ...
            for r_id, direction in enumerate(ray_dirs):
                ...
                for center, radius in zip(self.obstacle_centers, self.obstacle_radii):
                    hit_distance = self._ray_circle_intersection_distance(
                        ...
                        direction=direction,
                        center=center,
                        radius=radius + self.cfg.uav_radius,
                        max_range=max_range,
                    )
    ...
    def _compute_target_observations(self) -> np.ndarray:
        """
        g_i = [distance_to_assigned_target, relative_bearing_to_assigned_target], normalized.
        """
    ...
    def _compute_topology_observations(self) -> np.ndarray:
        """
        q_i = relative positions of all teammates.

        For each agent i, concatenate:
            [dx_i0, dy_i0, dx_i1, dy_i1, ...] excluding i itself.

        If use_communication_range_mask=True, teammates outside communication_range
        are represented as [0, 0]. Otherwise all teammates are included.
        """
    ...
    # ---------------------------------------------------------------------
    # Geometry, reward, termination, diagnostics
    # ---------------------------------------------------------------------
    ...
    def _compute_placeholder_rewards(
        self,
        ...
        """Simple placeholder reward. Full hierarchical reward will be added later."""
...
if __name__ == "__main__":
    ...
    print("\nObservation layout:")
    print(f"  z_i LiDAR:              [0:{cfg.lidar_num_rays}]")
    print(f"  u_i ego motion:         [{cfg.lidar_num_rays}:{cfg.lidar_num_rays + 4}]")
    print(f"  g_i target feature:     [{cfg.lidar_num_rays + 4}:{cfg.lidar_num_rays + 6}]")
    print(f"  q_i swarm topology:     [{cfg.lidar_num_rays + 6}:{env.obs_dim}]")
